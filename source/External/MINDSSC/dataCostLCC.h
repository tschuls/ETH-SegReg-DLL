...
void prepNCC(float* meanvar1,float* vol1,int r,int m,int n,int o){
    ...
    for(int i=0;i<sz;i++){
        ...
        meanvar1[i+sz*2]=sqrt(max(float(0.0),var1[i]));
    }
...
void *costVolFilterLCC(void *threadarg)
{
                ...
                for(int j=0;j<n1;j++){
                    for(int i=0;i<m1;i++){
                        ...
                        if(l2<len3){ //ensure no empty labels (for four-alignment) are used
                            ...
                            float ncc=1.0f;
                            //divide by product of variances
                            if(fabs(meanvar1[tarind+sz1*2])>0.0f&fabs(meanvar2[movind+szp*2])>0.0f)
                                ncc=1.0f-min(
                                             max(
                                                 (float)(covar)/(meanvar1[tarind+sz1*2]*meanvar2[movind+szp*2]),-1.0f)
                                             ,1.0f); 
...
void dataRegLCC(float* costvol,float* target,float* warped1,int hw,int sparse,int r,float h1,int i_m, int i_n, int i_o){
    image_m=i_m;image_n=i_n;image_o=i_o;
    timeval time1,time2;
    int m=image_m; int n=image_n; int o=image_o;
    int sz=m*n*o;
    int m1=m/sparse; int n1=n/sparse; int o1=max(o/sparse,1);
    int sz1=m1*n1*o1;
    ...
    pthread_create(&thread1,NULL,costVolFilterLCC,(void *)&cost1);
    pthread_create(&thread2,NULL,costVolFilterLCC,(void *)&cost2);
    pthread_create(&thread3,NULL,costVolFilterLCC,(void *)&cost3);
    pthread_create(&thread4,NULL,costVolFilterLCC,(void *)&cost4);
